...
```bash
npm run dev:api       # development (ts-node-dev, hot reload)
npm run build:api     # compile TypeScript
npm run start:api     # run compiled JS
```
...
```bash
docker build -t smartdoc-api apps/api
docker run -p 4000:4000 --env-file apps/api/.env smartdoc-api
```
